#------------------------------------------------------
# R Server Code for the Capstone Project Prediction App
#------------------------------------------------------
...
# 1. firstly, use a FourGram; the first three words of which are the last three words of the user provided sentence
#    for which are trying to predict the next word. The FourGram is already sorted from highest to lowest frequency
# 2. If no FourGram is found, then back off to ThreeGram (first two words of ThreeGram last two words of the sentence)
# 3. If no FourGram is found, then back off to TwoGram (first word of TwoGram last word of the sentence)
# 4. If no TwoGram is found, then back off to OneGram (the most common word with highest frequency)
#
PredNextTerm <- function(inStr)
{
    ...
    if (!nxtTermFound & inStrLen > 0)
    {
        ...
        mesg <- "There are no predicted word found, kindly try another word."
    }
...
shinyServer(function(input, output) {
        ...
        output$text1 <- renderText({
          paste("You have entered this text: ", input$inputString)});
